...
export default function TimelineProgress({ items }: TimelineProgressProps) {
  ...
  const lineHeight = shouldAnimate
      ...
      <div className="container">
        ...
        <div className="relative mt-16">
          ...
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-1 -translate-x-1/2">
            {/* Background line */}
            <div className="absolute inset-0 bg-gradient-to-b from-cyan-500/20 to-violet-600/20"></div>

            {/* Animated progress line */}
            {shouldAnimate && (
              <motion.div
                className="absolute inset-0 bg-gradient-to-b from-cyan-500 to-violet-600"
          ...
          <div className="space-y-8">
            {items.map((item, idx) => {
              const itemProgress = shouldAnimate
                ? useTransform(scrollYProgress, [idx * 0.15, (idx + 1) * 0.15], [0, 1])
                : 1;

              const itemScale = shouldAnimate
                ? useTransform(itemProgress, [0, 0.5, 1], [0.8, 1.1, 1])
                : 1;

              const itemOpacity = shouldAnimate
                ? useTransform(itemProgress, [0, 0.3, 1], [0, 1, 1])
                : 1;
