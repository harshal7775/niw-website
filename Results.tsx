...
export default function Results() {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="absolute inset-0 -z-10">
          <motion.div
            ...
            style={
              shouldAnimate
                ? {
                    y: useTransform(metricsProgress, [0, 1], [0, 80]),
                  }
                : {}
            }
          />
        ...
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {metrics.map((item, idx) => {
              ...
              const metricProgress = shouldAnimate
                ? useTransform(metricsProgress, [idx * 0.1, (idx + 1) * 0.1 + 0.15], [0, 1])
                : 1;

              const metricScale = shouldAnimate
                ? useTransform(metricProgress, [0, 0.5, 1], [0.8, 1.1, 1])
                : 1;

              const metricOpacity = shouldAnimate
                ? useTransform(metricProgress, [0, 0.3, 1], [0, 1, 1])
                : 1;
      ...
      <section ref={caseStudiesRef} className="py-24 bg-gradient-to-b from-white/30 to-white/10">
        <div className="container">
          <SectionHeading
          ...
          <div className="mt-16 space-y-12">
            {caseStudies.map((study, idx) => {
              const caseProgress = shouldAnimate
                ? useTransform(caseStudiesProgress, [idx * 0.15, (idx + 1) * 0.15 + 0.1], [0, 1])
                : 1;

              const caseScale = shouldAnimate
                ? useTransform(caseProgress, [0, 0.4, 1], [0.9, 1.02, 1])
                : 1;

              const caseOpacity = shouldAnimate
                ? useTransform(caseProgress, [0, 0.2, 1], [0, 1, 1])
                : 1;

              const caseY = shouldAnimate
                ? useTransform(caseProgress, [0, 0.3], [40, 0])
                : 0;
